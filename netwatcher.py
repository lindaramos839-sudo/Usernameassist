...
# -------------------------
# Network monitoring (scapy or psutil fallback)
# -------------------------
def collect_ips_scapy(args):
    """Sniff packets using scapy and extract IPs."""
            ...
            if pkt.haslayer(scapy.IP):
                ...
                for ip in [src_ip, dst_ip]:
                    if ip and not is_private_ip(ip):
                        record_connection_event(ip)
                        cnt = recent_conn_count(ip)
                        if cnt >= SUSPICIOUS_CONN_THRESHOLD:
                            if ip not in suspicious_ips or ip in CUSTOM_WATCHLIST:
                                enqueue_enrichment(ip, f"scapy threshold {cnt}", 
                                                 do_nmap=args.enable_nmap, 
                                                 scan_consent=args.scan_consent)
        except Exception as e:
            print("Packet handler error:", e)
    
    print(f"[{now_ts()}] Starting scapy sniffing (requires root)...")
    try:
        scapy.sniff(prn=packet_handler, store=False)
    except Exception as e:
        print(f"Scapy sniffing failed: {e}")

def collect_ips_psutil(args):
    """Poll network connections using psutil."""
    if not psutil:
        print("psutil not available, cannot monitor connections")
        return
    
    print(f"[{now_ts()}] Starting psutil polling...")
    while True:
        try:
            ...
            for conn in conns:
                if conn.raddr:
                    ...
                    if ip and not is_private_ip(ip):
                        ...
                        if cnt >= SUSPICIOUS_CONN_THRESHOLD:
                            if ip not in suspicious_ips or ip in CUSTOM_WATCHLIST:
                                enqueue_enrichment(ip, f"psutil threshold {cnt}",
                                                 do_nmap=args.enable_nmap,
                                                 scan_consent=args.scan_consent)
...
# Live dashboard (rich)
# -------------------------
def dashboard_loop(args):
    """Display live terminal dashboard."""
    if not RICH_AVAILABLE:
        print("rich library not available, dashboard disabled")
        return
    
    console = Console()
    def generate_table():
        table = Table(title="NetWatcher - Suspicious IPs")
        table.add_column("IP", style="cyan")
        table.add_column("Hostname", style="magenta")
        table.add_column("Country", style="green")
        table.add_column("ISP", style="yellow")
        table.add_column("Recent Conns", style="red")
        table.add_column("Timestamp", style="blue")
        
        with _lock:
            for ip, data in list(suspicious_ips.items())[:20]:
                hostname = str(data.get("hostname") or "N/A")[:30]
                geo = data.get("geo") or {}
                country = "N/A"
                isp = "N/A"
                if isinstance(geo, dict) and geo.get("status") == "success":
                    country = geo.get("country") or "N/A"
                    isp = geo.get("isp") or "N/A"
                conns = str(recent_conn_count(ip))
                ts = data.get("timestamp") or "N/A"
                table.add_row(ip, hostname, country, isp, conns, ts[:19])
        return table
...
def create_flask_app():
    ...
    def authenticate():
        return Response(
            ...
            {'WWW-Authenticate': 'Basic realm="NetWatcher"'})
    ...
    def index():
        ...
        <html>
        <head>
            <title>NetWatcher Dashboard</title>
        ...
        <body>
            <h1>NetWatcher - Suspicious IP Dashboard</h1>
            <div class="export">
                <a href="/export/csv">Export to CSV</a> | 
                <a href="/api/ips">JSON API</a>
            </div>
            <table>
                <tr>
                    <th>IP</th>
        ...
        with _lock:
            for ip, data in suspicious_ips.items():
                ...
                ips_data.append((ip, {
                    ...
                    "country": country,
                    "isp": isp,
                    "conns": recent_conn_count(ip),
                    "timestamp": (data.get("timestamp") or "")[:19]
                }))
        return render_template_string(html, ips=ips_data)
    ...
    def export_csv():
        output = []
        output.append("IP,Hostname,Country,ISP,Recent Connections,Timestamp\n")
        with _lock:
            for ip, data in suspicious_ips.items():
                geo = data.get("geo") or {}
                country = "N/A"
                ...
                output.append(f"{ip},{hostname},{country},{isp},{conns},{ts}\n")
        return Response("".join(output), mimetype="text/csv",
                       headers={"Content-Disposition": "attachment;filename=netwatcher_export.csv"})
    
    return app

def run_flask_dashboard(args):
    """Run Flask web dashboard."""
    app = create_flask_app()
    if app:
        port = int(os.getenv("FLASK_PORT") or 5000)
        host = os.getenv("FLASK_HOST") or "127.0.0.1"
        print(f"[{now_ts()}] Starting Flask dashboard on {host}:{port}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="NetWatcher - Network Monitor & Suspicious IP Investigator")
    parser.add_argument("--mode", choices=["scapy", "psutil", "dashboard", "flask"], default="psutil",
                       help="Monitoring mode (default: psutil)")
    parser.add_argument("--enable-nmap", action="store_true",
                       help="Enable nmap scanning (requires --scan-consent)")
    parser.add_argument("--scan-consent", action="store_true",
                       help="Consent to active network scanning (required for nmap)")
    parser.add_argument("--workers", type=int, default=WORKER_COUNT,
                       help=f"Number of worker threads (default: {WORKER_COUNT})")
    ...
    print(f"[{now_ts()}] NetWatcher starting...")
    print(f"  Mode: {args.mode}")
    print(f"  Workers: {WORKER_COUNT}")
    print(f"  nmap: {'enabled' if args.enable_nmap else 'disabled'}")
    print(f"  Scan consent: {'yes' if args.scan_consent else 'no'}")
    
    if args.enable_nmap and not args.scan_consent:
        print("[!] WARNING: --enable-nmap requires --scan-consent")
        args.enable_nmap = False
    ...
    print(f"[{now_ts()}] Started {WORKER_COUNT} worker threads")
    
    # Start monitoring based on mode
    try:
        if args.mode == "scapy":
            if SCAPY_AVAILABLE:
                collect_ips_scapy(args)
            else:
                print("scapy not available, falling back to psutil")
                collect_ips_psutil(args)
        elif args.mode == "psutil":
            collect_ips_psutil(args)
        elif args.mode == "dashboard":
            dashboard_loop(args)
        elif args.mode == "flask":
            run_flask_dashboard(args)
    except KeyboardInterrupt:
        print(f"\n[{now_ts()}] Shutting down...")
    finally:
        # Stop workers
        ...
        for t in worker_threads:
            t.join(timeout=2)
        print(f"[{now_ts()}] NetWatcher stopped")
